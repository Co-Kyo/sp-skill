...
export const initialize = step('initialize', '初始化')
  ...
  .inputs('raw_input')
  .action('parse', 'init-parse', '解析输出目录', '解析用户指定目录或默认 {当前日期}-{场景简称}，确认目录可用。')
  ...
  .initRules(
    { title: '确认 workDir', body: '向用户展示将要使用的输出目录并等待确认；如果用户未指定，默认使用 {当前日期}-{场景简称} 作为 workDir；用户可修正 workDir 路径' },
    { title: '初始化记录', body: 'workDir 确认后写入 {workDir}/.meta/init.json，后续所有步骤的产出路径均基于此目录派生' },
  )
  .detail(`初始化只负责对齐 workDir 和公共规则：
...
5. 后续步骤按需加载公共规则，不再重复确认 workDir。`)
  .verify(
    verify.file('{workDir}/.meta/init.json', '初始化记录存在'),
    verify.json('{workDir}/.meta/init.json', '初始化记录可解析'),
    verify.field('workDir', '初始化记录包含 workDir'),
    verify.field('confirmed', '初始化记录包含确认状态'),
    verify.file('{workDir}/.meta/run/run.json', '运行信封存在'),
    verify.json('{workDir}/.meta/run/run.json', '运行信封可解析'),
  )
  .onFail(
