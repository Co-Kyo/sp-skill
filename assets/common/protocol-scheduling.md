...
| 计数单位 | Task Group | 1 个命题 = 1 个 Task Group（命题组装步骤特殊：1 命题 = 2 agent） |

## 各步骤调度模式一览

> **注**：意图锚定、依赖分区、能力图谱、评估入池为串行主线程步骤，不涉及子 agent 调度，故不在此表中列出。能力图谱(capability-graph)的信源补搜使用 `web_search` + `web_fetch` 直接在主线程完成。
...
**命题组装特殊处理**：1 个命题 = 2 个 agent（Markdown + Experiment），两者独立可并行。
- 槽位计数：1 个命题占 2 个槽位
...
约束：
- 能力研究(capability-research)的 task **全部内联**（能力信息在分组时已确定，不读外部文件）
- Briefing 组装/命题组装/学习阶梯的 task **指定文件路径**（前置步骤产出量大，用 read 工具按需读取）
